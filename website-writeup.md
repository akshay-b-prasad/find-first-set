...
I built three independent SystemVerilog implementations from scratch — sequential FSM, parallel combinational, and pipelined binary search — each targeting a different point in the area/latency/throughput design space. All three were synthesized with Yosys against an ASAP7-approximate 7nm library and verified across 66,536 test vectors in Verilator.
...
The design decision worth highlighting: `result` and `no_set` are driven **directly in RUNNING** rather than through staging registers. An earlier draft buffered them into `result_r` and `no_set_r` and forwarded them in DONE. That cost 7 extra flip-flops for zero functional benefit — the outputs are held stable in DONE anyway.
...
The naive approach is a `casez` priority encoder. That synthesizes to 64 gate levels for W=64 — an O(N) chain. At 7nm (~18 ps/level), that's ~1.15 ns, capping Fmax below 870 MHz before wire delay or setup time. It fails 1 GHz timing with no margin.
...
## Architecture 3 — Pipeline: 6-Stage Binary Search
...
The trimming from 432 to 90 FFs deserves a note: `pipe_win` is padded to W bits at every stage, but synthesis eliminates the zero-padded upper bits since they carry no information. By stage 3, only 8 bits of the window are live. Yosys + ABC reduced 432 theoretical FFs to 90 actual.
...
**Latency and Fmax are not the same thing.** The sequential design has the shallowest critical path of the three (7 levels, ~7.1 GHz) while having the worst latency (up to 66 cycles). High Fmax means the clock can tick fast — it says nothing about how many ticks an answer takes. This surprises most people when they reason through it for the first time.

**`generate` is not a shorthand — it is how parallel hardware exists.** A for-loop in RTL produces a chain. `generate` produces parallel structures. There is no software analog: you cannot write a for-loop that creates independent parallel hardware. Understanding this is the difference between RTL that synthesizes to what you intended and RTL that synthesizes to something O(N) slower.
